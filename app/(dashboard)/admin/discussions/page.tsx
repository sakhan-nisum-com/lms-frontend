    ...
    <DashboardLayout role="admin" userName="Morgan Patel">
      <div className="space-y-6 max-w-6xl">
        ...
        <div className="space-y-3">
          {filtered.map((t) => (
            <div key={t.id} className="rounded-2xl p-4 flex items-start gap-3" style={{ backgroundColor: "#1E293B", border: "1px solid #334155" }}>
              <div
                className="flex items-center justify-center w-9 h-9 rounded-full text-xs font-bold text-white flex-shrink-0"
                style={{ backgroundColor: t.authorRole === "instructor" ? "#8B5CF6" : "#3B82F6" }}
              >
                {t.authorAvatar}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  {t.isPinned && <Pin size={12} style={{ color: "#F59E0B" }} fill="#F59E0B" />}
                ...
                <p className="text-xs mt-1 line-clamp-2" style={{ color: "#94A3B8" }}>{t.body}</p>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  ...
                  style={{ backgroundColor: t.isPinned ? "#F59E0B20" : "#33415560", color: t.isPinned ? "#FCD34D" : "#94A3B8" }}
                >
                  <Pin size={14} />
                </button>
                <button
                  ...
                  className="flex items-center justify-center w-8 h-8 rounded-lg"
                  style={{ backgroundColor: t.isSolved ? "#10B98120" : "#33415560", color: t.isSolved ? "#34D399" : "#94A3B8" }}
                >
          ...
          {filtered.length === 0 && (
            <div className="rounded-2xl p-10 text-center" style={{ backgroundColor: "#1E293B", border: "1px dashed #334155" }}>
              <p className="text-sm" style={{ color: "#475569" }}>No discussion threads match these filters.</p>
            </div>
          )}
        </div>
      </div>
