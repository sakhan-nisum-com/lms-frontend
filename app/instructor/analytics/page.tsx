    ...
    <div className="flex items-end gap-2 h-36 pt-2">
      {data.map((d, i) => (
        <div key={d.day} className="flex-1 flex flex-col items-center gap-1.5 relative">
          ...
          />
          <span className="text-xs" style={{ color: "#64748B" }}>{d.day}</span>
        </div>
      ))}
    </div>
  )
              ...
              >
                {RANGES.map((r) => (
                  <button
                    ...
                    style={{ color: r === range ? "#60A5FA" : "#94A3B8" }}
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            >
              ...
              <p className="text-xs mt-1" style={{ color: "#10B981" }}>{sub}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-semibold text-white">Enrollments</h3>
                <p className="text-xs mt-0.5" style={{ color: "#64748B" }}>New students per day</p>
              </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-semibold text-white">Completions</h3>
                <p className="text-xs mt-0.5" style={{ color: "#64748B" }}>Courses finished per day</p>
              </div>
          ...
          <div className="divide-y" style={{ borderColor: "#334155" }}>
            {TOP_COURSES.map((course) => (
              ...
              >
                ...
                <p className="text-sm text-white text-right">{course.students.toLocaleString()}</p>
                <p className="hidden sm:block text-sm text-right" style={{ color: "#94A3B8" }}>{course.completions.toLocaleString()}</p>
                <p className="hidden md:block text-sm font-semibold text-white text-right">{course.revenue}</p>
                <div className="hidden sm:flex items-center justify-end gap-1 text-sm" style={{ color: "#F59E0B" }}>
                  <Star size={12} fill="#F59E0B" />
                  {course.rating}
                </div>
                <p className="text-sm font-semibold text-right" style={{ color: "#10B981" }}>{course.trend}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: "Completion Rate",     value: 74, color: "#3B82F6", desc: "% of enrolled students who finished" },
            { label: "Student Retention",   value: 68, color: "#10B981", desc: "% who returned after first lesson"   },
            ...
            >
              ...
              <div className="h-2 rounded-full overflow-hidden mb-2" style={{ backgroundColor: "#334155" }}>
