    ...
    <div className="rounded-2xl p-5 shadow-sm" style={{ backgroundColor: "var(--bg-surface)", border: "1px solid var(--border-default)" }}>
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="min-w-0">
          <p className="text-sm font-bold truncate" style={{ color: "var(--text-primary)" }}>{group.name}</p>
          {trainingTitle && <p className="text-xs truncate" style={{ color: "var(--text-tertiary)" }}>{trainingTitle}</p>}
        </div>
      </div>

      <p className="text-xs mb-3 leading-relaxed" style={{ color: "var(--text-secondary)" }}>{group.description}</p>
      ...
      <button
        ...
        style={
          isMember
            ? { backgroundColor: "var(--border-default)", color: "var(--text-secondary)" }
